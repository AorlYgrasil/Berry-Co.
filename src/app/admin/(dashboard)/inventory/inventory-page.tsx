...
// Inline fallback for InlineStockAdjust to avoid missing-module errors.
// Keep this lightweight; the real component can be placed in a separate file.
function InlineStockAdjust({ productId, stock }: { productId: string; stock: number }) {
  return (
    <button
      type="button"
      className="inline-flex items-center rounded-md bg-stone-100 px-3 py-1 text-xs font-medium text-stone-700 hover:bg-stone-200"
      aria-label={`Adjust stock for ${productId}`}
    >
      Adjust ({stock})
    </button>
  )
