...
export async function getCart(owner: CartOwner) {
  ...
  const { data: items, error } = await supabase
    .from('cart_items')
    .select('id, product_id, quantity, unit_price_snapshot, products(id, name, price, image_url)')
    .eq('cart_id', cart.id)
  ...
  const normalizedItems = (items ?? []).map((item) => {
    ...
    return { ...item, product: product ?? null, unit_price_snapshot: Number(item.unit_price_snapshot) }
  })
...
export async function addToCart(owner: CartOwner, productId: string, quantity: number) {
  ...
  const request = existing
    ? supabase.from('cart_items').update({ quantity: newQuantity, unit_price_snapshot: product.price }).eq('id', existing.id)
    : supabase.from('cart_items').insert({ cart_id: cart.id, product_id: productId, quantity, unit_price_snapshot: product.price })
...
export async function updateCartItemQuantity(owner: CartOwner, cartItemId: string, quantity: number) {
  ...
  const { data, error } = await supabase.from('cart_items').update({ quantity, unit_price_snapshot: product.price }).eq('id', cartItemId).select('*').single()
